function [spot] = compSpotPosDevFmin4(image, ff, backer, back_int,...
    Istd_, im_mask, MAX_NUM_SPOT, CONST, channel, disp_flag, fig_num, opt)
...
% load the images. The images are enlarged because fitting improves when
% there are many pixels per spot
% im_mask_ is an enlarged cell mask for more accurate spot fitting
...
% fluorescent image is segmented to to obtain regions around each spot.
im_smoothed = imfilter(image,gauss,'replicate');


im_loc_raw = zeros( imsize(1)+2*crop,imsize(2)+2*crop);
im_loc_raw(crop+1:crop+imsize(1),crop+1:crop+imsize(2)) = im_smoothed;
...
% Go through each watershed region and record the intensity of its spot.
...
% only the brightest two watersheds are fit
MIN_AREA = 8;
...
if ~isempty(iis)
    ...
    for ii = iis
        ...
        xpos = ddd(1); % x position of fit foci
        ypos = ddd(2); % y position of fit foci
        IG   = ddd(3); % intensity
        B    = ddd(4); % width of fitted gaussian
        ...
        % only record the spot if the fit looks good (score>0)
        if score > 0
            spot_num = spot_num+1;
            rr(:,i) = [xpos,ypos];
...
% loop through the spots found and fix the standard deviations
...
for ii = 1:numel( spot )
    ...
    % do_the_fit : does the foci fitting
