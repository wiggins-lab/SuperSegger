...
%     if FLAGS.P_flag
%         if FLAGS.v_flag
%             im(:,:,1) = im(:,:,1) + 0.5*ag(data.cell_outline);
%         else
%             im(:,:,1) = im(:,:,1) + 0.5*ag(data.outline);
%         end
%     end
%     
    if FLAGS.P_flag
    ...
    if isfield( data, 'fluor1' ) && ...
            ( FLAGS.f_flag < 0 ||  FLAGS.f_flag == 1 );
        
        if FLAGS.filt(1) && isfield( data, 'flour1_filtered' )
    ...
    if isfield( data, 'fluor2' )  && ...
            ( FLAGS.f_flag < 0 ||  FLAGS.f_flag == 2 );
        
        if FLAGS.filt(2) && isfield( data, 'flour2_filtered' )
            ...
            if isfield( data.CellA{kk}, 'locus1') && ...
                ...
                for mm = 1:num_spot
                    ...
                    text_ = [num2str(data.CellA{kk}.locus1(mm).score, '%0.1f'),...
                              ' ', ...
                             num2str(data.CellA{kk}.locus1(mm).b, '%0.1f')];
...
%                         text( r(1)+x_+1, r(2)+y_, text_, 'Color',[0.2,0.2,0.2] );
%                         plot( r(1)+x_, r(2)+y_, 'o','MarkerSize',3,'Color',[0.2,0.2,0.2] );
                    end
                end
            ...
            if isfield( data.CellA{kk}, 'locus2') && ...
                ...
                for mm = 1:num_spot
                    ...
                    text_ = [num2str(data.CellA{kk}.locus2(mm).score, '%0.1f'),...
                              ' ', ...
                             num2str(data.CellA{kk}.locus2(mm).b, '%0.1f')];
