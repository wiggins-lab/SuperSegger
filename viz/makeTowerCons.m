...
% T0 is the number of frames in the consensus, 8 is typical
T0 =  8;
...
for ii = 1:numframe
    ...
    [imCell{ii}, maskCell{ii}, alpha(ii), ssCell{ii}, xxCell{ii}, yyCell{ii}] = ...
        intDoCellOri( data.CellA{ii}, W0, mag );
       
    % set the max and min fluor values
    if isempty(f1mm)
        if isfield( data.CellA{ii}, 'fluor1mm' )
            f1mm = data.CellA{ii}.fluor1mm;
        else
            f1mm = [ min(data.CellA{ii}.fluor1(:)), max(data.CellA{ii}.fluor1(:))];
        end
    else
        if isfield( data.CellA{ii}, 'fluor1mm' )
            f1mm = [ min([data.CellA{ii}.fluor1mm(1),f1mm(1)]), max([data.CellA{ii}.fluor1mm(2),f1mm(2)]) ];
        else
            f1mm = [ min([f1mm(1),min(data.CellA{ii}.fluor1(:))]), max([f1mm(2),max(data.CellA{ii}.fluor1(:))])];
        end
    end
end
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [fluor1, mask_rot, alpha, ss, xx, yy] = intDoCellOri( celld, W0, mag )
...
if isfield( celld, 'fl1') && isfield( celld.fl1, 'bg' ) && ~isnan( celld.fl1.bg)
    bg_fluor =  double(celld.fl1.bg);
else
    bg_fluor = 0;
end
...
for ii = x
    if (xsumt(ii)>0) && (xsum(ii)>0)
        ...
        mask_rot_(:,ii) = interp1( dy1, mask_rot(:,ii), dy2,'linear','extrap' );
...
for ii = 1:nt
    ...
    maskCell_{ii} = interp2( (X-xcom)/dx*2*L0, Y-ycom, maskCell{ii}, Xi-1, Yi-1);
