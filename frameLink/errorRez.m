...
for regNum =  1 : data_c.regs.num_regs;
    ...
    if data_c.regs.ID(regNum) == 0
        ...
        elseif numel(mapCR) == 1 && numel(data_r.regs.map.f{mapCR}) == 2
            ...
            otherSister = mapRC (mapRC~=regNum);
            
            if numel(otherSister) == 1 && any(mapRC==regNum) && data_c.regs.map.r{otherSister} == mother
                sister1 = regNum;
            ...
            elseif numel(otherSister) == 1 && any(mapRC==regNum) && data_c.regs.map.r{otherSister} ~= mother
