...
for regNum =  1 : data_c.regs.num_regs;
    ...
    elseif ismember (regNum,modRegions)
        ...
        cCellsFromR = data_r.regs.map.f{cellsInRfromC};
        rCellsTransp = data_r.regs.map.f{cCellsFromR};
        
        numberMatch = numel(cellsInRfromC) == numel(rCellsTransp) && ...
            numel(cCellsFromR) == numel(cellsInCreverse);
        assignmentMatch = numberMatch && all(cCellsFromR == cellsInCreverse) && ...
            all(cellsInRfromC == rCellsTransp);
      ...
      elseif numberMatch
          ...
          if numel(rCellsFromC) == 1 && numel(cCellsFromR)  == 1 &&  numel (cCellsTransp) == 2
            %% c <-> r mapping inconsistent numbers
