function [data,A]  = superSeggerOptiLasso(phase_, mask, disp_flag, CONST, adapt_flag, header, crop_box )
...
%image.
if ismember('100X',CONST.ResFlag)
...
for ii = 1:numSegs
    ...
    score (ii) = calculateLassoScores (seg_info(ii,:),A);
    %scoreRaw(ii) = segmentScoreFunLasso( seg_info(ii,:), A );
