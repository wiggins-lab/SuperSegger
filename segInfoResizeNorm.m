function seg_info = segInfoResizeNorm(segs_props, segs_props_tmp, regs_prop, regs_label,disk1, pixelFactor)
...
dil3 = round(3/pixelFactor);
if nn>2
    mask_ii_end  = (compConn(seg_mask,4)==1);
    mask_tmp = xor(seg_mask,mask_ii_end);
    mask_tmp_dil = imdilate(mask_tmp,ones(dil3));
    mask_ii_out  = xor(mask_tmp_dil,seg_mask);
elseif nn == 1
    mask_tmp_dil = imdilate(seg_mask,ones(dil3));
    mask_ii_out  = xor(mask_tmp_dil,seg_mask);
else
