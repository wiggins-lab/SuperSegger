function seg_info = segInfoResizeCurv19(segs_props, segs_props_tmp, regs_prop, regs_label,disk1, pixelFactor)
...
[seg_info(1),ind] = min(phase_im(:).*double(seg_mask(:))+1e6*double(~seg_mask(:)));
...
seg_info(4) = mean(phase_im(mask_ii_out)) - seg_info(2);
...
% min pixel
seg_info(5) = mean(phase_im(ii_min_norm))-mean(phase_im(ii_min_para));

% seg_info(:,6) is the second d of the phase parallel to the seg at the
% min pixel
tmp_mask = xor(ii_min_para,min_pixel);
seg_info(6) = mean(phase_im(tmp_mask))-seg_info(1);
