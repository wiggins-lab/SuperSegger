...
seg_info(4) = (mean(segs_props_tmp.phaseC2(mask_ii_out)) - seg_info(2))/pixelfactor^2;
...
% min pixel
seg_info(5) = (mean(segs_props_tmp.phaseC2(ii_min_norm))-mean(segs_props_tmp.phaseC2(ii_min_para)))/pixelfactor^2;

% seg_info(:,6) is the second d of the phase parallel to the seg at the
% min pixel
tmp_mask = xor(ii_min_para,min_pixel);
seg_info(6) = (mean(segs_props_tmp.phaseC2(tmp_mask))-seg_info(1))/pixelfactor^2;
