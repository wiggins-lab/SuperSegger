...
%       numc : fluorescence channel number
% OUTPUT : 
...
im_rel = flourFiltered/dI0; % normalization so that intensities
% so that 
...
tmp = im_rel-1;
tmp(tmp<0) = 0; % logical mask of foci found
L = watershed(-tmp); % watershed to identify foci
Lp = logical(double(L).*double(mask_mod));

fr_label = bwlabel(Lp);
props = regionprops( fr_label, {'BoundingBox'} );
num_foci_regs = numel(props);
imsize = size(image);
...
% Loop through the foci regions and fit loci in each one
max_med_ii  = nan([1,num_foci_regs]);
max_disk_ii = nan([1,num_foci_regs]);
x_ii = nan([1,num_foci_regs]);
y_ii = nan([1,num_foci_regs]);
cell_ii = nan([1,num_foci_regs]);

for ii = 1:num_foci_regs

    [xx,yy] = getBBpad( props(ii).BoundingBox, imsize, 3 );
    [XX_ii,YY_ii] = meshgrid(xx,yy);
    
    ims_ii  = flourFiltered(yy,xx);  % filtered image 
    mask_ii = (fr_label(yy,xx)==ii); % foci region
