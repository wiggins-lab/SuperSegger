...
% defineGoodSegs : segragates segments into good (real) and bad (spurious).
% It uses intensity thresholds to segregate the set of segments 
% produced by the watershed algorithm into "good" segments (segs_good) 
% which lie along a real cellular boundary, and "bad" segments, which lie 
% along spurious boundaries within single cells.
...
for ii = 1:numSegs
    ...
    for kk = 1:numel(ind_reg);
        ...
        %[L1(kk),L2(kk)] = makeRegionSize( kk_mask,e1,e2, double(((segs_label(yymin_:yymax_, xxmin_:xxmax_) == ii))));
        [L1(kk),L2(kk)] = makeRegionSize( kk_mask,e1,e2);
        %[L1(kk),L2(kk)] = makeRegionSizeFast( kk_mask,segs_props(ii));
        %[L1(kk),L2(kk)] = makeRegionSizeProjection( kk_mask );
    end
